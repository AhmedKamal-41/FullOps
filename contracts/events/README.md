...
- **`aggregateId` is always the order ID**, for every event from every service — not each service's own internal identifier (a reservation ID, a payment ID, a fulfillment ID). Those still exist and are carried in `payload` when relevant. Keying every event by the order ID means: all events for one order land on the same Kafka partition (see the root `README`/`ARCHITECTURE.md` for the partitioning convention), or ordering is preserved for the whole saga, and Order Service's operations projection (ADR 0008) can build a per-order view by grouping on one field instead of correlating across four different ID spaces. **One documented exception**: `InventoryLowStock.v1` is a SKU-scoped signal, not an order-scoped one — there is no order to key by. Its `aggregateId` is a name-based UUID deterministically derived from the SKU (`UUID.nameUUIDFromBytes`), which still satisfies the envelope's UUID-shaped `aggregateId` field and keeps every event for one SKU on the same Kafka partition (so a SKU's low-stock crossings are never delivered out of order relative to each other); the real `sku` string is carried in `payload`.
...
| `InventoryLowStock.v1` | inventory-service | A SKU's available quantity crossed its configured low-stock threshold, in either direction. SKU-scoped, not order-scoped — see the `aggregateId` exception above. |
