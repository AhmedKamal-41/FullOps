...
/**
 * Proves OrderEventsListener's own @RetryableTopic wiring for its OrderPlaced handling.
 * ReservationService is mocked only for the transient-failure case; the malformed-payload case
 * exercises real production code (OrderEventsListener.parseItemsOrFailNonRetryably) with no mocking
 * needed.
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.MOCK)
@ActiveProfiles("test")
@Import({TestcontainersConfiguration.class, TestSecurityConfig.class})
class OrderEventsListenerRetryIT {
  ...
  @Test
  void aTransientFailureThatRecoversWithinTheRetryBudgetNeverReachesTheDlt() {
    when(reservationService.reserve(any(), any(), any(), any()))
        .thenThrow(new RuntimeException("simulated transient failure, attempt 1"))
        .thenThrow(new RuntimeException("simulated transient failure, attempt 2"))
        .thenReturn(new ReservationOutcome.Reserved(UUID.randomUUID(), List.of()));
    ...
    verify(reservationService, timeout(20_000).times(3)).reserve(any(), any(), any(), any());

    ConsumerRecord<String, String> onDlt =
        pollFor(orderEventsTopic + "-dlt", eventId.toString(), Duration.ofSeconds(5));
    assertThat(onDlt)
