 ...
 * (reserve stock) and OrderCancellationRequested (release whatever reservation this order still has
 * — a no-op if there is none, e.g. cancellation was requested before stock was ever reserved). Both
 * event types share one consumer group on this topic, since Kafka only delivers each message to one
