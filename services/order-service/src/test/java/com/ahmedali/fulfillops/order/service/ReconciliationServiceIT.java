 ...
 * threshold gets exactly one safe retry before escalating to REQUIRES_REVIEW, a happy-path order
 * stuck past its threshold escalates straight away, and the advisory lock actually stops two
 * concurrent reconcile() calls from double-processing the same stuck order.
 */
