...
import com.ahmedali.fulfillops.order.messaging.DeadLetterEvent;
 ...
 * original bytes and lets it reprocess successfully — while a non-ADMIN is forbidden, an unknown
 * event id 404s, and replaying the same event twice is rejected as a conflict rather than a silent
 * double-send.
 */
...
class DeadLetterReplayControllerIT {
  ...
  @Value("${app.messaging.fulfillment-events-topic}")
  ...
  void adminCanReplayADeadLetteredEventAndItReprocessesSuccessfully() throws Exception {
    ...
    ConsumerRecord<String, String> republished =
        pollForRecordWithKey(fulfillmentEventsTopic, orderId.toString());
  ...
  private void publishMalformedEvent(UUID eventId, UUID orderId) {
    ...
    kafkaTemplate.send(
        fulfillmentEventsTopic, orderId.toString(), objectMapper.writeValueAsString(envelope));
  }
